...
class VocosExp(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        periodicity_loss = np.array([x["periodicity_loss"].cpu() for x in outputs]).mean()
        pitch_loss = np.array([x["pitch_loss"].cpu() for x in outputs]).mean()
        f1_score = np.array([x["f1_score"].cpu() for x in outputs]).mean()
