...
class HydraCrossAttention(nn.Module):
    r"""
    A cross attention block that uses Hydra
    """
...
@maybe_allow_in_graph
class Mamba2TransformerBlock(nn.Module):
    r"""
    A Transformer block that replaced Attention with Mamba2.
...
@maybe_allow_in_graph
class FNetTransformerBlock(nn.Module):
    r"""
    A Transformer block with FNet encoder.
...
class HydraTransformerBlock(nn.Module):
    r"""
    A Transformer block that replaced Attention with Hydra.
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            self.attn2 = HydraCrossAttention(
                d_model=dim*2,
                headdim=attention_head_dim,
                bias=attention_bias,
                **kwargs
